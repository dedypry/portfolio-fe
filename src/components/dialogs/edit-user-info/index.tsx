...
const EditUserInfo = ({ open, setOpen, data }: EditUserInfoProps) => {
  ...
  return (
    ...
    >
      ...
      <DialogTitle className='flex gap-2 flex-col text-center sm:pbs-16 sm:pbe-6 sm:pli-16' variant='h4'>
        Add Education
      </DialogTitle>
      <form onSubmit={e => e.preventDefault()}>
                  ...
                  renderValue: selected => (
                    <div className='flex items-center gap-2'>
                      {(selected as string[]).map(value => (
                        <Chip key={value} className='capitalize' label={value} size='small' />
                      ))}
                    </div>
                  )
                }}
        ...
        <DialogActions className='justify-center pbs-0 sm:pbe-16 sm:pli-16'>
          ...
          <Button color='secondary' type='reset' variant='tonal' onClick={handleClose}>
            Cancel
          </Button>
        </DialogActions>
      </form>
